...
export function isLeftEyeOpen(face, threshold = 5) {
  ...
  let maxIrisY = _.maxBy(iris, p => p.y)
  let minIrisY = _.minBy(iris, p => p.y)
  ...
  let points = [
    ...
    MESH_ANNOTATIONS.leftEyeUpper3,
  ].flatMap(i => keypoints[i])

  let maxEyeY = _.maxBy(points, p => p.y)
  let minEyeY = _.minBy(points, p => p.y)
